...
// plus the text-size/line-height/radius/spacing :root declarations, into
// one generated.css — see vite.config.ts's generate-tokens-css plugin.
// Tailwind v4 only auto-generates utilities for tokens declared inside
// `@theme`; declaring a token only in a plain :root (as tokens.css's
// hand-authored colors/fonts/icon-size are) makes it available as a CSS
// custom property but not as a Tailwind utility, so every one of those
// also needs mirroring into `@theme inline`. Rather than hand-maintaining
// that mirror (the previous source of drift), this reads tokens.css's own
// hand-authored `:root` text and mirrors whatever custom properties it
// finds — nothing to keep in sync by hand for colors/fonts/icon-size
// either.
export function generateTokensCss(handAuthoredTokensCss: string): string {
  ...
  const spacing = Object.values(SPACING_SCALE);
  ...
  const spacingLines = spacing.map((e) => `  --spacing-${e.slug}: calc(var(--spacing) * ${e.multiplier}); /* ${e.comment} */`).join('\n');
  ...
  const themeSpacingLines = spacing.map((e) => `  --spacing-${e.slug}: var(--spacing-${e.slug});`).join('\n');
...
${radiusLines}
  ...
  /* Spacing — a single axis of presets (xs..3xl), used for any directional utility (px, py, gap, etc.) */
${spacingLines}
}
...
${handAuthoredMirrorLines}
  ...
  /* Text, leading, radius, and spacing — from src/lib/{textScale,radiusScale,spacingScale}.ts */
${themeSizeLines}
