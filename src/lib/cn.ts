...
// only recognizes sm/md/lg/xl/2xl/3xl/full/none) and for the spacing-inline/stack
// suffixes (tailwind-merge's padding/margin scales only recognize the stock numeric
// scale and arbitrary `[...]` values) — in each case, without registering the custom
// suffix, both the old and new class survive a `cn()` merge and whichever lands later
// in the compiled CSS wins, regardless of argument order. All three registrations are
...
const spacingSlugs = [
  ...Object.values(SPACING_INLINE_SCALE).map((e) => e.slug),
  ...Object.values(SPACING_STACK_SCALE).map((e) => e.slug),
];
...
const twMerge = extendTailwindMerge({
  extend: {
    classGroups: {
      ...
      // `pl-inline-md`, Page's `mb-stack-lg` previously weren't covered at
